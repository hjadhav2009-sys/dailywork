...
Production-ready foundation for a small ecommerce seller's daily Meesho warehouse workflow, now maintained in the new dailywork repository.

> This repository is the active development home for DailyWork Pick & Pack.
> Use hjadhav2009-sys/meesho only as the source/reference repo for comparison or copy work.
> Do not modify the old meesho repo from this workflow.
...
NEXT_PUBLIC_APP_NAME=DailyWork Pick & Pack
...
```text
scripts\windows\start-dailywork-app.bat
```
...
```powershell
scripts\windows\start-dailywork-app.ps1
```

Compatibility launchers still exist:

Legacy shortcut aliases: scripts\\windows\\start-meesho-app.bat and scripts\\windows\\start-meesho-app.ps1.
...
1. Start the app on the owner PC by double-clicking `scripts\windows\start-dailywork-app.bat`.
